...
// Plugin-related methods (called by AliasEditorWrapper)
impl VisualAliasEditor {
    pub fn plugin_save(&mut self, window: &mut Window, cx: &mut Context<Self>) -> Result<(), plugin_editor_api::PluginError> {
        if let Some(file_path) = &self.file_path {
            if let Some(root_block) = self.canvas.root_block() {
                if let Some(ast) = root_block.to_ast() {
                    ...
                    match serde_json::to_string_pretty(&asset) {
                        ...
                        Err(e) => {
                            Err(plugin_editor_api::PluginError::FileSaveError {
                                path: file_path.clone(),
                    ...
                    Err(plugin_editor_api::PluginError::Other {
                        message: "Type has empty slots - fill all slots before saving".into(),
                    })
                }
            } else {
                Err(plugin_editor_api::PluginError::Other {
                    message: "Cannot save empty type".into(),
                })
            }
        } else {
            Err(plugin_editor_api::PluginError::Other {
    ...
    pub fn plugin_reload(&mut self, window: &mut Window, cx: &mut Context<Self>) -> Result<(), plugin_editor_api::PluginError> {
        if let Some(file_path) = &self.file_path {
            match std::fs::read_to_string(&file_path) {
                Ok(json_content) => {
                    match serde_json::from_str::<ui_types_common::AliasAsset>(&json_content) {
                        Ok(asset) => {
                            self.name = asset.name.clone();
                            self.display_name = asset.display_name.clone();
                            self.description = asset.description.unwrap_or_default();

                            let root_block = TypeBlock::from_ast(&asset.ast);
                            self.canvas = BlockCanvas::with_root(root_block);
                            self.error_message = None;
                            self.preview_needs_update = true;
                            self.update_preview(window, cx);
                            cx.notify();
                        ...
                        Err(e) => {
                            Err(plugin_editor_api::PluginError::FileLoadError {
                                path: file_path.clone(),
